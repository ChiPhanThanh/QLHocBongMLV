...
namespace QLHocBongMLV
{
    public partial class QLDuHoc : Form
    {
        ...
        private void ToExcel_DuHoc(DataGridView dataGridViewDH, string fileName)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dataGridViewDH.ColumnCount; i++)
                {
                    worksheet.Cells[1, i + 1] = dataGridViewDH.Columns[i].HeaderText;
                }
                // export nội dung trong DataGridView
                for (int i = 0; i < dataGridViewDH.RowCount; i++)
                {
                    for (int j = 0; j < dataGridViewDH.ColumnCount; j++)
                    {
                        worksheet.Cells[i + 2, j + 1] = dataGridViewDH.Rows[i].Cells[j].Value.ToString();
                    }
                }
        ...
        private void btnXuatExcel_Click(object sender, EventArgs e)
        {
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                ToExcel_DuHoc(dataGridViewDH, saveFileDialog1.FileName);
            }
        }
    }
}
